...
Original file is located at
    https://colab.research.google.com/drive/1NhVIvAwzivXRRB6iZOJJHTfBUpUBt0Jn
...
def compute_FastText_embeddings(pd_dataframe, embedding_dim):
    ...
    embedding_vec = np.vstack([b,b,embedding_vec])
    ### TODO ###
...
class RnnEncoder(nn.Module):
    def __init__(self, pretrained_emb, vocab_size, embedding_dim, hidden_units):
        ...
        pretrained_emb1 = torch.from_numpy(pretrained_emb)  
        pretrained_emb1 = pretrained_emb1.type(torch.FloatTensor)
        self.embed = nn.Embedding.from_pretrained(pretrained_emb1)
        self.gru = nn.GRU(input_size = embedding_dim, hidden_size = hidden_units, num_layers = 1, bidirectional = False, batch_first = False)
...
class RnnDecoder(nn.Module):
    def __init__(self, pretrained_emb, vocab_size, embedding_dim, hidden_units):
        ...
        self.gru = nn.GRU(input_size = (embedding_dim + hidden_units), hidden_size = hidden_units, num_layers = 1, bidirectional = False, batch_first = True)

        self.full = nn.Linear(hidden_units, vocab_size)
    ...
    def compute_attention(self, dec_hs, enc_output):
        ...
        a = dec_hs.permute(1,0,2)
        b = enc_output.permute(1,0,2)
        c = self.w1(a)
        d = self.w2(b)
        e = F.tanh(c+d)
        attention_weights = self.v(e)
        attention_weights = F.softmax(attention_weights, dim= 1)
        f = attention_weights * b
        context_vector = torch.sum(f, dim = 1)
    ...
    def forward(self, x, dec_hs, enc_output):
        ...
        concatenate = torch.cat((embed,context_vector), dim = 2)
        output, dec_hs = self.gru(concatenate)
        fc_out = self.full(output)
...
class TransformerEncoder(nn.Module):
    def __init__(self, pretrained_emb, src_vocab_size, embedding_dim, num_heads,
        ...
        a = nn.TransformerEncoderLayer(nhead= num_heads, dim_feedforward=dim_feedforward, d_model = embedding_dim)
        self.trans = nn.TransformerEncoder(a, num_layers = num_layers)
    ...
    def forward(self, x):
        ...
        output = None
        a = self.embed(x)
        b = self.dropout(a + self.position_embedding)
        mask = self.make_src_mask(x)
        output = self.trans(b, src_key_padding_mask = mask)
...
class TransformerDecoder(nn.Module):
    def __init__(self, pretrained_emb, trg_vocab_size, embedding_dim, num_heads,
        ...
        pretrained_emb1 = torch.from_numpy(pretrained_emb)  
        pretrained_emb1 = pretrained_emb1.type(torch.FloatTensor)
        self.embed = nn.Embedding.from_pretrained(pretrained_emb1).to(device)   

        self.dropout = nn.Dropout().to(device)   

        a = nn.TransformerDecoderLayer(nhead= num_heads, dim_feedforward=dim_feedforward, d_model = embedding_dim)
        self.trans = nn.TransformerDecoder(a, num_layers = num_layers).to(device)   

        self.full = nn.Linear(embedding_dim, trg_vocab_size).to(device) 
        self.device = device  
        ### TODO ###
    ...
    def forward(self, dec_in, enc_out):
        ...
        """
        output = None
        a = self.embed(dec_in).to(self.device)   
        c = self.position_embedding[:dec_in.shape[0],:].to(self.device)   
        b = self.dropout(a + c).to(self.device)   
        trg_mask = self.generate_square_subsequent_mask(dec_in.shape[0]).to(self.device)   
        output = self.trans(b, memory = enc_out, tgt_mask = trg_mask).to(self.device)   
        output = self.full(output).to(self.device)   
